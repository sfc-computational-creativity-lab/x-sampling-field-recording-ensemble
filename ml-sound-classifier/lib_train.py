            ...
            for i in range(1, 100 + 1):
                try:
                    ...
                    if generated.shape[0] != i or generated.shape[2] != k:
                        print('Fail at i={}, k={}, generated.shape={}'.format(
                            i, k, generated.shape))
                except:
...
def mels_build_multiplexed_X(conf, X_files):
    ...
    XX = np.zeros((len(X_files), conf.samples_per_file,
                   conf.dims[0], conf.dims[1]))
    for i, file in enumerate(X_files):
        whole_mels = read_as_melspectrogram(conf, file, trim_long_data=False)
        multiplexed = mels_populate_samples(
            whole_mels, conf.dims[1], conf.samples_per_file)
        XX[i, :, :, :] = multiplexed
...
def mels_demux_XX_y(XX, y):
    ...
    # ex. if dims[1]==2) [1, 2, 3] -> [1, 1, 2, 2, 3, 3]
    y = np.repeat(y, dims[1], axis=0)
    return X, y
...
def get_class_distribution(y):
    ...
    sample_distribution = {cur_cls: len(
        [one for one in y_cls if one == cur_cls]) for cur_cls in classset}
...
def _balance_class(X, y, min_or_max, sampler_class, random_state):
    ...
    sampler_instance = sampler_class(
        ratio=flat_ratio, random_state=random_state)
...
def print_class_balance(title, y, labels):
    ...
    zeroclasses = [label for i, label in enumerate(
        labels) if i not in distributions.keys()]
...
def get_steps_per_epoch(conf, _Xtrain):
    train_steps_per_epoch = (
        len(_Xtrain) + conf.batch_size - 1) // conf.batch_size
...
def balance_dataset(conf, X, y):
    if conf.data_balancing == 'over_sampling' or conf.data_balancing == 'under_sampling':
        print_class_balance(
            ' <Before> Current category distribution', y, conf.labels)
        ...
        print(' Dataset is not balanced so far, conf.data_balancing =',
              conf.data_balancing)
    return X, y
...
def summarize_metrics_history(metrics_history, show_graph=True):
    ...
    if show_graph:
        plt.plot('x', 'f1', data=df, marker='',
                 color='blue', markersize=2, linewidth=1)
        plt.plot('x', 'recall', data=df, marker='', color='olive', linewidth=1)
        plt.plot('x', 'precision', data=df, marker='o',
                 color='pink', markerfacecolor='red', linewidth=4)
        plt.plot('x', 'accuracy', data=df, marker='',
...
def evaluate_model(conf, model, X, y):
    ...
    if conf.samples_per_file > 1 and conf.eval_ensemble:
        ...
        f1, recall, precision, acc = calculate_metrics(
            conf, one_y, ensemble_preds)
...
def train_classifier(conf, fold, dataset, model=None, init_weights=None,
    ...
    train_generator = create_train_generator(
        conf, Xtrain, ytrain, image_data_generator)
    ...
    callbacks = [
        ...
        TensorBoard(log_dir=str(
            datapath(conf, 'logs/fold_%d' % fold)), write_graph=True)
    ]
    if model is None:
